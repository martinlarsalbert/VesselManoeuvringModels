...
Y_qs_eq = sp.Eq(
    ...
    #    + p.Yuuv * u ** 2 * v
    #    + p.Yuur * u ** 2 * r
    + p.Yuv * u * v + p.Yur * u * r + p.Ydelta * delta + p.Ydeltadeltadelta * delta ** 3
    #    + p.Yudelta * u * delta
...
N_qs_eq = sp.Eq(
    ...
    # + p.Nvdeltadelta * v * delta ** 2 + p.Nrdeltadelta * r * delta ** 2
...
vmm_gsi = VMM(X_eq=X_eq, Y_eq=Y_eq, N_eq=N_eq)
